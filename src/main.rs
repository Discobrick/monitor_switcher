...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    TranslateMessage, WNDCLASSW, WM_DESTROY, WM_DEVICECHANGE, WS_OVERLAPPED, CreateWindowExW,
    MSG, WINDOW_EX_STYLE,
};
...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Current Directory set to: {}", exe_dir.display());
    ...
    // SAFETY: GetModuleHandleW is called with None to get the handle of the current process.
    // This is safe because it doesn't modify any state and only returns the process base address.
    let instance = unsafe {
    ...
    // SAFETY: RegisterClassW and CreateWindowExW are called with valid parameters.
    // The class name and window proc are defined within this module.
    unsafe {
        ...
        if hwnd.0 == 0 {
            return Err("Failed to create background window".into());
        }
    }
    ...
    // SAFETY: The message loop is standard Win32. GetMessageW will block until a message is available.
    // It returns 0 on WM_QUIT and -1 on error.
    unsafe {
        loop {
            ...
            if result.0 == 0 {
                ...
                return Err("GetMessageW returned -1".into());
            }
    ...
    match msg {
        WM_DEVICECHANGE => {
            ...
            if wparam.0 == 0x0007 || wparam.0 == 0x8000 || wparam.0 == 0x8004 {
                let Ok(last_trigger_guard) = LAST_TRIGGER.lock() else {
                    println!("[Error] LAST_TRIGGER mutex is poisoned");
                    return LRESULT(0);
                };

                // Check if we are still in the cooldown period
                if let Some(instant) = *last_trigger_guard
                    && instant.elapsed() < COOLDOWN_DURATION
                {
                    println!("[Debug] Ignoring event due to cooldown.");
                    return LRESULT(0);
                }
                drop(last_trigger_guard);
        ...
        WM_DESTROY => {
            // SAFETY: PostQuitMessage is safe to call when a window is being destroyed.
            unsafe { PostQuitMessage(0) };
            LRESULT(0)
        }
        // SAFETY: DefWindowProcW is the standard way to handle unprocessed messages.
...
fn check_usb_state() {
    ...
    println!("[Debug] State - Any Found: {any_found}, Previously Connected: {previously_connected}");
...
fn execute_commands(args_list: &[String]) {
    ...
    for args_str in args_list {
        println!("[System] Tool: {MONITOR_TOOL} | Args: {args_str}");
